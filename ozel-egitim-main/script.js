...
// --- REHBER AVATAR FONKSİYONLARI ---
function moveAvatarTo(elementId) {
    ...
    if (avatar && target) {
        ...
        // Avatarı butonun biraz sol üstüne konumlandır
        ...
        // Zıplama efekti
        avatar.style.transform = "scale(1.1) translateY(-10px)";
        setTimeout(() => {
...
function resetIdleTimer() {
    ...
    idleTimer = setTimeout(() => {
        ...
        moveAvatarTo('nextBtn'); // Bekleme süresi dolunca ileriye yönlendir
...
async function loadNext() {
    if(isWaiting) return;
    clearTimeout(idleTimer);
    hideAvatar(); // Yeni video yüklenirken avatarı gizle
    document.getElementById('nextBtn').classList.remove('pulse-anim');
            ...
            vEl.onloadeddata = () => {
                ...
                setTimeout(() => { 
                    ...
                    speak(currentObj.q, () => {
                        ...
                        moveAvatarTo('micBtn'); // 🎙️ Soru bitince mikrofona git
                        resetIdleTimer();
...
async function rec() {
    clearTimeout(idleTimer);
    hideAvatar(); // Dinleme başlarken avatarı çek
    document.getElementById('micBtn').disabled = true;
    ...
    r.onresult = async (e) => {
        ...
        if (turnCount >= 7) {
            ...
            speak(final, () => {
                document.getElementById('nextBtn').classList.add('pulse-anim');
                moveAvatarTo('nextBtn'); // ➡️ Sohbet bitince ileriye yönlendir
        ...
        speak(aiRes, () => {
            document.getElementById('micBtn').disabled = false;
            moveAvatarTo('micBtn'); // AI konuşması bitince tekrar mikrofona yönlendir
            isWaiting = false;
            resetIdleTimer();
            document.getElementById('info').innerText = "Konuşmak için mikrofona bas!";
        });
